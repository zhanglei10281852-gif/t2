...
const Inventory: React.FC = () => {
  ...
  const fetchRecords = async () => {
    try {
      setLoading(true);
      const response = await inventoryApi.getRecords({ record_type: activeTab });
  ...
  const inColumns = [
    { title: '单号', dataIndex: 'order_no', key: 'order_no', width: 150 },
    ...
    { title: '入库日期', dataIndex: 'record_date', key: 'record_date', width: 120 },
      ...
      >
        ...
        {currentStep === 0 && (
          <Form form={form} layout="vertical">
            <Form.Item name="warehouse_id" label="选择仓库" rules={[{ required: true }]}>
                  ...
                  <Select placeholder="选择物资" showSearch optionFilterProp="children">
                    {materials.map((m) => (
                      <Option key={m.code} value={m.code}>
                        {m.code} - {m.name} (库存: {m.quantity}{m.unit})
                      </Option>
                    ))}
                  </Select>
                </Form.Item>
        ...
        {currentStep === 2 && (
          <Form form={form} layout="vertical">
            <Row gutter={16}>
              <Col span={12}>
                <Form.Item name="order_no" label="单号" rules={[{ required: true }]}>
            ...
            {activeTab === 'out' && (
              <>
                ...
                <Form.Item name="purpose" label="用途描述" rules={[{ required: true }]}>
